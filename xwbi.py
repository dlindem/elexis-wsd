...
wbi_config['MEDIAWIKI_API_URL'] = "https://unidive.wikibase.cloud/w/api.php"
wbi_config['SPARQL_ENDPOINT_URL'] = "https://unidive.wikibase.cloud/query/sparql"
wbi_config['WIKIBASE_URL'] = "https://unidive.wikibase.cloud/"
...
def packstatements(statements, wbitem=None, qualifiers=False, references=False):
    ...
    for statement in statements:
        ...
        if statement['type'].lower() == "string":
            packed_statement = String(value=statement['value'].strip(), prop_nr=statement['prop_nr'],
            ...
            packed_statement = Time(time=statement['value'], precision=statement['precision'],
        ...
        for site in importitemjson['sitelinks']:
            if site.replace('wiki', '') in languages_to_consider:
                wpurl = "https://" + site.replace('wiki', '') + ".wikipedia.org/wiki/" + \
                        importitemjson['sitelinks'][site]['title']
                print(wpurl)
                wbitemjson['statements'].append({'prop_nr': config.wd_sitelinks_prop, 'type': 'url', 'value': wpurl})

    wbitemjson[
        'qid'] = wbqid  # if False, then create new item. If wbqid given, prop-values are transferred to that item using action 'keep' [existing values]
